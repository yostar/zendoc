...
function isXcodeSelectError(err) {
...
function showWorkspaceReadyPanel(workspacePath) {
    const panel = vscode.window.createWebviewPanel('zendoc.workspaceReady', 'Zendoc workspace ready', vscode.ViewColumn.One, { enableScripts: true });
...
function getWorkspaceReadyHtml() {
    return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
async function getGitHubTokenFromAuth() {
            ...
            try {
                ...
                const installZendocFromVsix = async () => {
                    ...
                    if (fs.existsSync(downloadsDir)) {
                        ...
                        if (zendocVsix)
                            candidates.push(path.join(downloadsDir, zendocVsix));
                    }
